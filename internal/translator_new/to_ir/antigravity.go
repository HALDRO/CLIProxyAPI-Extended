...
func FilterInvalidThinkingBlocks(messages []ir.Message) []ir.Message {
	...
	for _, msg := range messages {
		...
		for _, part := range msg.Content {
			if part.Type == ir.ContentTypeReasoning {
				...
				if cache.HasValidSignature(part.ThoughtSignature) {
...
func RemoveTrailingUnsignedThinking(messages []ir.Message) []ir.Message {
	result := make([]ir.Message, 0, len(messages))
