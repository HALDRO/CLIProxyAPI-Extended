...
func (h *OllamaAPIHandler) handleOllamaChatStream(c *gin.Context, openaiRequest []byte, modelName string) {
	...
	dataChan, _, errChan := h.ExecuteStreamWithAuthManager(cliCtx, constant.Ollama, modelName, openaiRequest, h.GetAlt(c))
...
func (h *OllamaAPIHandler) handleOllamaChatNonStream(c *gin.Context, openaiRequest []byte, modelName string) {
	...
	resp, _, errMsg := h.ExecuteWithAuthManager(cliCtx, constant.Ollama, modelName, openaiRequest, h.GetAlt(c))
	if errMsg != nil {
		h.WriteErrorResponse(c, errMsg)
